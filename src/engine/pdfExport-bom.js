 ...
 * pdfExport-bom.js
 *
 * Cut-list and nesting-plan PDF generation — the "bom" branch's own
 * export file, split out of the old monolithic engine/pdfExport.js so
 * this domain never needs to touch what engine/pdfExport-joints.js,
 * -history.js or -assembly.js own.
 *
 * formatDim/rowCell/drawRow/COLUMNS and the nesting-specific drawing
 * helpers below are used ONLY within this file, so they stayed local
 * rather than moving to pdfExport-shared.js — see that file's header
 * for what's actually shared vs. domain-specific.
 */

import { jsPDF } from 'jspdf';
import { PAGE_MARGIN_MM, HEADER_BLOCK_HEIGHT_MM, drawTableRow, outputPdf } from './pdfExport-shared.js';
...
/**
 * @param {ReturnType<typeof import('./nesting.js').nestCutList>} nestResults
 * @param {ReturnType<typeof import('./nesting.js').summarizeNestingResult>} summary
 * @param {{projectName?:string, fileName?:string, mode?:'save'|'open'}} [options]
 */
