  ...
  bomBodyEl.innerHTML = rows
    .map(
      ...
      <tr>
        ...
        <td class="right">${Math.round(row.width)} × ${Math.round(row.height)}</td>
        <td>${row.material}</td>
        <td class="right">${row.thickness}mm</td>
        <td title="${row.grainDirection ? `Grain runs with this piece's ${row.grainDirection}` : 'No grain constraint'}">${row.grainLabel}</td>
        <td title="${row.edgeBandingLabel === '—' ? 'No banded edges' : `Banded: ${row.edgeBandingLabel}${row.edgeBandingMaterial ? ' (' + row.edgeBandingMaterial + ')' : ''}`}">${row.edgeBandingLabel}</td>
        <td class="right">${row.quantity}</td>
        <td class="right">${row.areaTotalM2.toFixed(3)}</td>
      </tr>`
    )
    .join('');
...
function updateSelectedField(field, value) {
  ...
  if (field === 'material') {
    ...
    // A grainDirection only means anything for a material that
    // hasGrain (see properties.js's grainDirectionHTML, which hides
    // the control entirely otherwise) — clear it on a switch to a
    // non-grain material so a stale value can't linger and show up
    // in the cut list for a material it no longer applies to.
    const patch = { material: catalogEntry.name, thickness: catalogEntry.thicknessMm };
    if (!catalogEntry.hasGrain) patch.grainDirection = null;
...
function addBox() {
  ...
  // Builds the exact same kind of relation the collinear TOOL creates
  // for its "MOVE" case (see applyCollinear above) — an ordinary
  // `attachedTo` constraint pinning `myFace` of the panel it ends up
  // on to `targetFace` of `targetNode`, live and persistent. Used
  // below for every one of Top/Bottom/Back/Front's positions relative
  // to left/right, instead of writing that constraint shape out by
  // hand four times — so the box's own construction goes through the
  // same "make these two faces collinear" primitive a user gets from
  // the Collinear button, and modifying left/right (an ordinary drag,
  // the resizeProxy redirect, or collinear itself acting on left/
  // right again later) automatically carries through to whichever
  // panel is collinear-attached to them, exactly like any other
  // attachedTo relation in this app.
  ...
  const top = createPanelNode({
    ...
    constraints: [
      ...
      collinearTo('positionY', 'front', left, 'top'),
    ],
  });
  const bottom = createPanelNode({
    ...
    constraints: [
      ...
      collinearTo('positionY', 'back', left, 'bottom'),
    ],
  });
  const back = createPanelNode({
    ...
    constraints: [
      ...
      collinearTo('positionZ', 'front', left, 'right'),
    ],
  });
  const front = createPanelNode({
    ...
    constraints: [
      ...
      collinearTo('positionZ', 'back', left, 'left'),
    ],
  });
