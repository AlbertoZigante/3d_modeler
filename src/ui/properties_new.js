...
import { getDisplayName, MATERIAL_CATALOG, getMaterialInfo } from '../modeller/modules.js';
...
export function renderProperties(
  ...
  container.innerHTML = `
    ...
    ${grainDirectionHTML(selectedPanel)}
    ${edgeBandingHTML(selectedPanel)}
  ...
  const grainField = container.querySelector('#grain-direction-field');
  if (grainField) {
    grainField.addEventListener('change', (e) => {
      onFieldChange('grainDirection', e.target.value || null);
    });
  }
  container.querySelectorAll('.edge-banding-checkbox input').forEach((checkbox) => {
    checkbox.addEventListener('change', (e) => {
      const nextBanding = { ...(selectedPanel.edgeBanding || { top: false, bottom: false, left: false, right: false }) };
      nextBanding[e.target.dataset.edge] = e.target.checked;
      onFieldChange('edgeBanding', nextBanding);
    });
  });
  const bandingMaterialField = container.querySelector('#edge-banding-material-field');
  if (bandingMaterialField) {
    bandingMaterialField.addEventListener('change', (e) => {
      onFieldChange('edgeBandingMaterial', e.target.value || null);
    });
  }
...
function grainDirectionHTML(selectedPanel) {
  const materialInfo = getMaterialInfo(selectedPanel.material);
  if (!materialInfo?.hasGrain) return ''; // this material has no visible direction — a grain constraint wouldn't mean anything, so don't offer one at all rather than show a control that does nothing
  const value = selectedPanel.grainDirection || '';
  return `
    <div class="field-row">
      <label>Grain direction</label>
      <select id="grain-direction-field" class="field-input">
        <option value="" ${value === '' ? 'selected' : ''}>— Unconstrained —</option>
        <option value="width" ${value === 'width' ? 'selected' : ''}>Runs with Width</option>
        <option value="height" ${value === 'height' ? 'selected' : ''}>Runs with Height</option>
      </select>
    </div>`;
}

function edgeBandingHTML(selectedPanel) {
  const banding = selectedPanel.edgeBanding || { top: false, bottom: false, left: false, right: false };
  const anyBanded = Object.values(banding).some(Boolean);
  return `
    <div class="field-row">
      <label>Edge banding</label>
      <div class="edge-banding-grid">
        ${['top', 'bottom', 'left', 'right'].map((edge) => `
          <label class="edge-banding-checkbox">
            <input type="checkbox" data-edge="${edge}" ${banding[edge] ? 'checked' : ''} />
    ...
    ${anyBanded ? `
    <div class="field-row">
      <label>Banding material</label>
      <div class="field-input-wrap">
        <input type="text" id="edge-banding-material-field" class="field-input" value="${escapeHtml(selectedPanel.edgeBandingMaterial || '')}" placeholder="e.g. PVC White 0.4mm" />
      </div>
    </div>` : ''}`;
}
